...
class CheckPassword {
    ...
    public function check() {
		...
		if ($this->minNums) {
            $pattern = '/\d/';
            $found = preg_match_all($pattern, $this->password, $matches);  //(301)
        ...
        return $this->errors ? false : true;  //(297)check() method returns false if theres any error, Otherwise, it returns true 
    }
	...
	public function requireMixedCase() { //(300)
        $this->mixedCase = true;
    }
	
	public function requireNumbers($num = 1) {  //(300)
